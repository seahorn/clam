...
static bool isSafeForWeakArrayDomains(const Cell& c, const DataLayout& dl) {
  if (isOverlappingCell(c, dl)) {
    CRAB_LOG("heap-abs",
      errs() << "\tCannot be disambiguated because overlaps with other cells.\n";);
    return false;
  }
  ...
  if (offset >= n->size()) {
    CRAB_LOG("heap-abs",
      errs() << "\tCannot be disambiguated because cell is out-of-bounds.\n";);    
    return false;
  }
...
region_info canBeDisambiguated(const Cell& c, const DataLayout& dl,
  ...
  if (!n->isModified() && !n->isRead()) {
    CRAB_LOG("heap-abs",
	     errs() << "\tWe do not bother to disambiguate it because "
  ...
  if (n->isIntToPtr() || n->isPtrToInt()) {
    if (!disambiguate_ptr_cast) {
      CRAB_LOG("heap-abs", 
	       errs() << "\tCannot be disambiguated: node is casted "
	                    << "from/to an integer.\n";);
      return region_info(UNTYPED_REGION, 0);
    }
  }
  ...
  if (isTypedCell(n, offset, int_pred) || isTypedArrayCell(n, offset, int_pred)) {
    if (isSafeForWeakArrayDomains(c, dl)) {
      ...
      CRAB_LOG("heap-abs", 
      errs() << "\tCannot be disambiguated because it's not safe weak array domains\n";);
      return region_info(UNTYPED_REGION, 0);      
    } 
  } 
