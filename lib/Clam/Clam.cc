...
namespace clam {
  ...
  using assertion_property_checker_t = crab::checker::assert_property_checker<intra_analyzer_t>;  
  // -- inter-procedural analysis
  using inter_params_t = crab::analyzer::top_down_inter_analyzer_parameters<cg_ref_t>;        
  using inter_analyzer_t = crab::analyzer::top_down_inter_analyzer<cg_ref_t, clam_abstract_domain>;
  // -- path analyzer
  using path_analyzer_t = crab::analyzer::path_analyzer<cfg_ref_t, clam_abstract_domain>;  
  ...
  private:
    ...
    void crabAnalyze(const AnalysisParams &params,
	...
	if (params.print_invars) {
	  ...
	  if (!params.keep_shadow_vars) {
	    shadow_varnames = std::vector<varname_t>(m_vfac.get_shadow_vars().begin(),
