...
template <typename Set> inline void set_difference(Set &s1, Set &s2) {
  Set s3;
  boost::set_difference(s1, s2, std::inserter(s3, s3.end()));
  std::swap(s3, s1);
}

template <typename Set> inline void set_union(Set &s1, Set &s2) {
  Set s3;
...
static const llvm::Value *getTypedSingleton(const llvm::DSNode *n,
                                            Pred &is_typed) {
  if (!n)
    return nullptr;
...
static bool isTypedArray(const llvm::DSNode *n, unsigned o, Pred &is_typed) {
  ...
  if (std::distance(n->type_begin(), n->type_end()) != 1)
    return false;
...
static region_info canBeDisambiguated(const llvm::DSNode *n, unsigned offset,
  ...
  if (n->isUnknownNode()) {
    if (!disambiguate_unknown) {
      CRAB_LOG("heap-abs",
               llvm::errs()
                   << "\tCannot be disambiguated: node is unknown.\n";);
      return region_info(UNTYPED_REGION, 0);
    }
  }
  ...
  if (n->isIncompleteNode()) {
    if (!disambiguate_external) {
      CRAB_LOG("heap-abs",
               llvm::errs()
                   << "\tCannot be disambiguated: node is incomplete.\n";);
      return region_info(UNTYPED_REGION, 0);
    }
  }
...
template <typename Set>
static void inputReachableNodes(const llvm::DSCallSite &cs, llvm::DSGraph &dsg,
                                Set &set) {
  markReachableNodes(cs.getVAVal().getNode(), set);
  if (cs.isIndirectCall())
    markReachableNodes(cs.getCalleeNode(), set);
  ...
  for (auto &gv :
       boost::make_iterator_range(sm.global_begin(), sm.global_end()))
...
static void argReachableNodesFromCall(llvm::DSCallSite CS, llvm::DSGraph &dsg,
...
void LlvmDsaHeapAbstraction::cacheReadModNewNodes(const llvm::Function &f) {
  ...
  for (const llvm::DSNode *n : reach) {
    ...
    for (auto &kv :
         boost::make_iterator_range(n->type_begin(), n->type_end())) {
      unsigned o = kv.first;
      region_info r_info =
          canBeDisambiguated(n, o, m_disambiguate_unknown,
...
// the new nodes are disjoint from mod nodes.
void LlvmDsaHeapAbstraction::cacheReadModNewNodesFromCallSite(
    llvm::CallInst &I) {
...
LlvmDsaHeapAbstraction::getRegion(const llvm::Function &F, llvm::Value *V) {
  ...
  region_info r_info =
      canBeDisambiguated(n, cell.getOffset(), m_disambiguate_unknown,
                         m_disambiguate_ptr_cast, m_disambiguate_external);
