...
class LowerGvInitializers : public ModulePass {
  ...
  void CreateStoreIntValue(Value &Base, const std::vector<APInt> &Indices,
                           const ConstantInt &Val, IRBuilder<> &Builder) {
    LOWERGV_LOG(errs() << "\tCreating Store with GEP with base=" << Base
                       << " and indices=\n";
  ...
  Constant *getInitFn(Type *type, std::vector<Constant *> &LLVMUsed,
                      Module &m) {
  ...
  Function *CreateZeroInitializerFunction(Value *V,
    ...
    if (m_cg)
      m_cg->getOrInsertFunction(fun);
    return fun;
  }

  void CreateZeroInitializerCallSite(Value &base,
    ...
    LOWERGV_LOG(
        errs() << "\tCreating zeroInitializer function: creating GEP with base="
               << base << " and indices=\n";
        for (unsigned i = 0, sz = Indices.size(); i < sz; ++i) {
          errs() << "\t\t Bitwidth=" << Indices[i].getBitWidth()
                 << " Val=" << Indices[i] << "\n";
  ...
  Constant *getIntInitFn(Type *ty, std::vector<Constant *> &LLVMUsed,
                         Module &m) {
  ...
  Function *CreateIntInitializerFunction(GlobalVariable &gv,
    ...
    Function *fun = dyn_cast<Function>(
        getIntInitFn(gv.getInitializer()->getType(), LLVMUsed, M));
    // XXX: do not mark it as ReadNone, otherwise LLVM will optimize
    // it away.
    // fun->addFnAttr(Attribute::ReadNone);
    if (m_cg)
  ...
  void CreateIntInitializerCallSite(GlobalVariable &gv, IRBuilder<> &Builder,
                                    std::vector<Constant *> &LLVMUsed,
                                    Module &M) {
  ...
  bool LowerConstantAggregateZero(Type *T, Value &base, IRBuilder<> &Builder,
    ...
    LOWERGV_LOG(errs() << "ConstantAggregateZero with Base=" << base.getName()
                       << " and Indices=";
                printIndices(Indices); errs() << " and Type=" << *T << "\n";);
    ...
    if (IntegerType *ITy = dyn_cast<IntegerType>(T)) {
      ...
      for (unsigned i = 0; i < STy->getNumElements(); ++i) {
        ...
        change |= LowerConstantAggregateZero(ETy, base, Builder, M, LLVMUsed,
                                             Indices);
        Indices.pop_back();
      }
      ...
      if (ATy->getElementType()->isIntegerTy()) {
        ...
        for (unsigned i = 0; i < ATy->getNumElements(); ++i) {
          ...
          // m_dl->getTypeAllocSize(ATy->getElementType()); uint64_t IndexSize =
          // ElementSize * 8;
          uint64_t IndexSize = 32;
          Indices.push_back(APInt(IndexSize, i));
          change |= LowerConstantAggregateZero(ATy->getElementType(), base,
  ...
  bool LowerInitializer(const Constant *C, Value &Base, IRBuilder<> &Builder,
    ...
    if (isa<ConstantPointerNull>(C) || isa<ConstantFP>(C) ||
      ...
      // m_dl->getTypeAllocSize(CP->getType()->getElementType()); uint64_t
      // IndexSize = ElementSize * 8;
      ...
      for (unsigned i = 0, e = CP->getNumOperands(); i < e; ++i) {
        APInt Index(IndexSize, i);
        Indices.push_back(Index);
        change |= LowerInitializer(CP->getOperand(i), Base, Builder, M,
                                   LLVMUsed, Indices);
      }
    } else if (isa<ConstantAggregateZero>(C)) {
      change |= LowerConstantAggregateZero(C->getType(), Base, Builder, M,
                                           LLVMUsed, Indices);
    } else if (const ConstantArray *CPA = dyn_cast<ConstantArray>(C)) {
      // unsigned ElementSize =
      // m_dl->getTypeAllocSize(CPA->getType()->getElementType()); uint64_t
      // IndexSize = ElementSize * 8;
      ...
      for (unsigned i = 0, e = CPA->getNumOperands(); i < e; ++i) {
        APInt Index(IndexSize, i);
        Indices.push_back(Index);
        change |= LowerInitializer(CPA->getOperand(i), Base, Builder, M,
                                   LLVMUsed, Indices);
      }
      ...
      for (unsigned i = 0, e = CPS->getNumOperands(); i < e; ++i) {
        ...
        APInt Index(IndexSize, i);
        Indices.push_back(Index);
        change |= LowerInitializer(CPS->getOperand(i), Base, Builder, M,
                                   LLVMUsed, Indices);
      }
...
public:
  ...
  virtual bool runOnModule(Module &M) {
    ...
    for (GlobalVariable &gv :
         llvm::make_range(M.global_begin(), M.global_end())) {
    ...
    */
    ...
    for (GlobalVariable *gv : gvs) {
      ...
      change |= LowerInitializer(gv->getInitializer(), *gv, Builder, M,
                                 MergedVars, Indices);
    }
