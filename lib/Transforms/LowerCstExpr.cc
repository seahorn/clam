...
bool lowerCstExprOnFunction(Function &F) {
...
bool lowerCstExprOnModule(Module &M) {
  bool change = false;
  for (auto &F : M) {
    change |= lowerCstExprOnFunction(F);
  }
  return change;
}
...
PreservedAnalyses LowerCstExprPass::run(Module &M, ModuleAnalysisManager &) {
  if (!lowerCstExprOnModule(M)) {
    return PreservedAnalyses::all();
  }
  // New instructions, but the CFG is untouched.
