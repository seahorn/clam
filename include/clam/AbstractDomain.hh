...
#define DUMP_TO_LLVM_STREAM(T)                                                 \
  inline llvm::raw_ostream &operator<<(llvm::raw_ostream &o, T &e) {           \
...
operator<<(llvm::raw_ostream &o, crab::domains::apron_domain<N, V, D> &inv) {
  crab::crab_string_os s;
  s << inv;
  o << s.str();
  return o;
...
inline llvm::raw_ostream &
operator<<(llvm::raw_ostream &o, crab::domains::elina_domain<N, V, D> &inv) {
  crab::crab_string_os s;
  s << inv;
  o << s.str();
  return o;
...
#define DEFINE_WRAPPER(WRAPPER, ABS_DOM, ID)                                   \
  ...
  public:                                                                      \
    ...
    ABS_DOM &get() { return m_abs; }                                           \
                                                                               \
    bool is_bottom() { return m_abs.is_bottom(); }                             \
                                                                               \
    bool is_top() { return m_abs.is_top(); }                                   \
                                                                               \
    void forget(const std::vector<var_t> &vars) { m_abs.forget(vars); }        \
                                                                               \
    void project(const std::vector<var_t> &vars) { m_abs.project(vars); }      \
                                                                               \
    ...
    void write(crab::crab_os &o) { m_abs.write(o); }                           \
                                                                               ...
                                                                               \
  template <>                                                                  \
  inline GenericAbsDomWrapperPtr mkGenericAbsDomWrapper(ABS_DOM abs_dom) {     \
