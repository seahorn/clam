...
public:  
  ...
  // Return the range for i-th operand ArgOperand of Inst before the
  // execution of Inst. The index 0 is for the LHS. The type of
  // ArgOperand should be either integer or pointer, otherwise an
  // error will be raised. If the returned range is the empty constant
  // range then the instruction is unreachable.
  virtual llvm::ConstantRange range(const llvm::Instruction &Inst, unsigned i) const = 0;
  ...
  virtual llvm::ConstantRange range(const llvm::BasicBlock &B, const llvm::Value &V) const = 0;
