...
public:
  constexpr Type(uint16_t value, llvm::StringRef name, llvm::StringRef description,
		 bool is_relational, bool is_disjunctive)
    : m_value(value), m_name(name), m_description(description),
      m_is_relational(is_relational),
      m_is_disjunctive(is_disjunctive) {}
  constexpr Type()
...
constexpr Type INTERVALS_CONGRUENCES(2,"ric", "intervals with congruences", false, false); 
...
constexpr Type DIS_INTERVALS(5, "dis-int", "cheap, non-overlapping intervals", false, true);
constexpr Type ZONES_SPLIT_DBM(6, "zones", "zones base on split normal form DBMs", true, false);
constexpr Type TERMS_INTERVALS(7, "term-int", "terms+intervals", true, false);
constexpr Type TERMS_DIS_INTERVALS(8, "term-dis-int", "terms+non-overlapping intervals", true, true);
...
constexpr std::array<Type, 11> List = {
 INTERVALS, INTERVALS_CONGRUENCES, WRAPPED_INTERVALS,
 BOXES, DIS_INTERVALS,
 ZONES_SPLIT_DBM, TERMS_INTERVALS, TERMS_DIS_INTERVALS, TERMS_ZONES,
 OCT, PK
};
