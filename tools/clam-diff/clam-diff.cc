...
llvm::cl::opt<clam::CrabDomain::Type, true, clam::CrabDomainParser> XClamDomain(
    "crab-dom",
    llvm::cl::desc("Crab numerical abstract domain used to compare invariants"),
    llvm::cl::values(
...
static llvm::cl::opt<std::string> In1(llvm::cl::Positional,
                                      llvm::cl::desc("<first JSON file>"),
                                      llvm::cl::Required,
                                      llvm::cl::value_desc("filename"));

static llvm::cl::opt<std::string> In2(llvm::cl::Positional,
                                      llvm::cl::desc("<second JSON file>"),
                                      llvm::cl::Required,
                                      llvm::cl::value_desc("filename"));

static llvm::cl::opt<bool> CompareInvariants(
    "compare-invariants",
    llvm::cl::desc("Whether compare semantically invariants (expensive)"),
    llvm::cl::init(true));
...
/** Semantic diff using inclusion abstract operation **/
static uint64_t diff_invariants(const std::string &function,
...
static void print_diff(const clam::json::global_analysis_results &gres1,
  ...
  for (auto &kv : gres1.m_results) {
    ...
    if (res1.safe_checks == res2.safe_checks &&
      ...
      diff << In1 << " | " << res1.safe_checks << " | " << res1.warning_checks
           << " | " << res1.error_checks << "\n";
      diff << In2 << " | " << res2.safe_checks << " | " << res2.warning_checks
           << " | " << res2.error_checks << "\n";
    }
  }
  ...
  if (CompareInvariants) {
    ...
    for (auto &kv : gres1.m_results) {
      ...
      regressions += impl_details::diff_invariants(function, res1.invariants,
                                                   res2.invariants, dom, diff);
    }
    diff << "Number of regressions found: " << regressions << "\n";
  }
...
int main(int argc, char *argv[]) {
  ...
  clam::json::parse_options clam_opts(CompareInvariants);
  std::unique_ptr<clam::json::global_analysis_results> res1 =
      clam::json::read_json(In1, clam_opts);
  std::unique_ptr<clam::json::global_analysis_results> res2 =
      clam::json::read_json(In2, clam_opts);
