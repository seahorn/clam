// RUN: %crabllvm --turn-undef-nondet --inline  --lower-select --devirt-functions=types --lower-unsigned-icmp --crab-do-not-print-invariants --crab-dom=boxes --crab-check=assert --externalize-addr-taken-functions %opts "%s" 2>&1 | OutputCheck -l debug %s
