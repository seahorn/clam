// RUN: %crabllvm --turn-undef-nondet --lower-unsigned-icmp --inline --devirt-functions=types --externalize-addr-taken-functions --crab-narrowing-iterations=2 --crab-widening-delay=2 --crab-widening-jump-set=0 --crab-check=assert --crab-stats  --crab-dom=boxes --crab-track=arr --crab-singleton-aliases  --crab-do-not-print-invariants "%s" 2>&1 | OutputCheck -l debug %s
